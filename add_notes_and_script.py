...
Ordre : 23 diapositives, justifications intégrées dans le fil principal."""
...
SCRIPT = [
 ...
 (5, "[EN] My role & context", "2:30", "EN",
  ...
  "These studies gave me the field knowledge of what the supervision actually had to monitor. Thank you — I "
  "will now continue in French."),
 (6, "Diagnostic & besoins", "1:15", "FR",
  ...
  "hiérarchisées et traduites en clair, une cartographie dynamique, et des notifications automatiques."),
 (7, "Le besoin en synthèse", "0:20", "FR",
  "Pour résumer ce besoin d’un seul regard : le pourquoi, c’est l’obsolescence et la criticité du parc ; le "
  "pour qui, ce sont d’abord les techniciens, mais aussi les clients et l’agence ; le comment, c’est toute la "
  "chaîne du routeur jusqu’au serveur ; et le périmètre, ce sont 16 clients et une trentaine de faisceaux."),
 (8, "Choix technologiques", "1:00", "FR",
  "Pour répondre à ce besoin, j’ai mené une véritable étude de marché, par filtres successifs, sur deux "
  "briques. D’abord le transport : j’ai comparé Cisco, Sierra Wireless et Teltonika, et j’ai retenu le routeur "
  ...
  "dans le groupe."),
 (9, "Justification des choix techniques", "0:25", "FR",
  "Je veux justifier rapidement mes principaux choix, car ils structurent tout le projet. Zabbix pour l’open "
  "source et sa souplesse ; Teltonika pour son cloud natif et son VPN ; un serveur physique parce que le devis "
  "cloud revenait plus cher sur la durée ; un polling à la minute pour voir l’évolution réelle du signal ; et "
  "les règles NoNat pour pouvoir identifier individuellement chaque faisceau. J’ai écarté Huawei et Robustel "
  "pour des raisons de souveraineté, conformément à l’ANSSI."),
 (10, "Planning & écarts", "1:00", "FR",
  "J’ai découpé le projet en cinq jalons séquentiels, chacun conditionnant le suivant. Le projet a pris quatre "
  "semaines de retard, mais je tiens à l’expliquer, car ces retards ne viennent pas d’une mauvaise estimation. "
  "Le premier jalon a pris sept semaines de plus : les fichiers techniques des équipements Ericsson n’étaient "
  "pas documentés, et personne en interne n’avait cette expertise — c’était impossible à anticiper. Le reste "
  "vient de 32 jours de missions urgentes pour d’autres clients, prioritaires car contractuelles. En retirant "
  "ces deux causes externes, le projet tombe exactement à la date prévue."),
 (11, "Cybersécurité — EBIOS RM", "1:00", "FR",
  ...
  "Pour le neutraliser, deux mesures : l’authentification multifacteur, qui bloque la compromission initiale, "
  "et un durcissement des routeurs, qui bloque tout rebond. La vraisemblance passe alors de très élevée à peu "
  "vraisemblable."),
 (12, "Pilotage : méthode & écarts", "0:30", "FR",
  "Un mot sur le pilotage. Pourquoi ce projet maintenant et pas avant ? Parce que l’intégration au groupe "
  "VINCI avait mobilisé les ressources internes jusque-là. Comment je justifie mes 1 001 heures de travail ? "
  "Par un calcul tâche par tâche, recoupé avec mon calendrier réel d’alternance. Et j’assume pleinement le "
  "retard : il s’explique entièrement par deux causes externes, sans dérive de ma méthode.\n\n"
  "[NOTE PERSO — NE PAS PROJETER : si on te pousse sur les chiffres, deux écarts existent dans le mémoire — "
  "déplacements évités 24/an côté ROI vs 32/an côté CO2 ; serveur 150 W côté électricité vs 200 W côté CO2. "
  "Réponse : ce sont des bornes basses/prudentes définies différemment selon le calcul ; la conclusion ne "
  "change pas.]"),
 (13, "Architecture matérielle", "1:00", "FR",
  ...
  "Internet déjà payée par le client — donc sans abonnement supplémentaire. Côté Axians, Zabbix collecte, "
  "Grafana visualise, le tout sur un serveur physique HPE. Le déploiement est Plug and Play : je préconfigure "
  "en atelier, puis la mise en service se fait à distance, sans déplacement."),
 (14, "Intégration Zabbix & MIB", "1:00", "FR",
  "Le cœur technique, et la partie la plus difficile, a été d’intégrer les équipements dans Zabbix. Les "
  "faisceaux Ericsson exposent leurs données via des centaines d’identifiants bruts, non documentés. J’ai dû "
  "capturer les trames directement sur les équipements pour identifier les bons. Avec des pièges : la "
  "puissance reçue est remontée en dixièmes — une valeur de moins 450 vaut en réalité moins 45. Sans "
  "correction, tout serait faux d’un facteur dix. J’ai aussi appris les expressions régulières pour décoder "
  "les alarmes. Le résultat, c’est un référentiel d’identifiants validés : un actif qui n’existait pas dans "
  "l’entreprise."),
 (15, "Durcissement & NoNat", "1:00", "FR",
  ...
  "tous les équipements derrière la sienne ; dans Zabbix, tous les faisceaux d’un site apparaîtraient alors "
  "identiques. Les règles NoNat désactivent ce masquage, uniquement pour la supervision : on préserve "
  "l’adresse de chaque faisceau et on identifie précisément lequel est en alarme, tout en gardant le "
  "cloisonnement."),
 (16, "Restitution & cartographie", "1:00", "FR",
  "Toutes ces données, il fallait les rendre lisibles. J’ai mis en place deux niveaux. Le niveau opérationnel, "
  "dans Zabbix, avec des panneaux hexagonaux qui affichent la puissance reçue en temps réel et passent au "
  "rouge en cas d’alarme. Et le niveau cartographique, sous Grafana, où chaque liaison est tracée sur une "
  "carte de France : le trait devient rouge quand un faisceau est en défaut. Pour cela, j’ai développé un "
  "script qui interroge la base de Zabbix et génère la carte automatiquement. Un technicien voit d’un coup "
  "d’œil quel site est en panne, et où."),
 (17, "IA locale", "0:45", "FR",
 ...
 (18, "Rentabilité (ROI)", "1:15", "FR",
  ...
  "Et ces chiffres reposent sur le scénario le plus prudent, le tarif plancher."),
 (19, "Les hypothèses du ROI", "0:25", "FR",
  "Je détaille les hypothèses qui soutiennent ces chiffres, parce qu’elles sont volontairement prudentes. Un "
  "taux horaire interne ; un tarif plancher, en dessous du marché ; un coût d’exploitation fixe quel que soit "
  "le nombre de clients. Et même dans le scénario où aucun déplacement ne serait évité, le projet reste "
  "rentable, à 159 %. Autrement dit, la réalité ne peut être que meilleure que ce que je présente."),
 (20, "Environnement & opérationnel", "1:00", "FR",
  ...
  "consomme : j’ai donc vérifié par le calcul. Le serveur émet 98 kilos de CO2 par an, contre 576 kilos évités "
  "par les déplacements supprimés : un bilan net positif de 478 kilos par an. Et il y a un troisième bénéfice, "
  "opérationnel : je compare désormais en temps réel la puissance mesurée d’une liaison à sa valeur théorique. "
  "Ce qui prenait plusieurs jours se vérifie en quelques minutes."),
 (21, "Les hypothèses du bilan CO2", "0:25", "FR",
  "Là aussi, je précise mes hypothèses : mes paramètres viennent de sources officielles — l’ADEME pour le "
  "véhicule, RTE pour l’électricité, HPE pour le serveur. La seule hypothèse libre, c’est la distance d’un "
  "déplacement ; je l’ai donc couverte par une analyse de sensibilité, de 50 à 200 kilomètres : le bilan reste "
  "positif dans tous les cas."),
 (22, "Bilan & perspectives", "1:00", "FR",
  ...
  "Pour l’entreprise, l’intérêt dépasse l’outil : c’est un actif durable, documenté et réutilisable. La "
  "principale perspective est d’élargir le parc : chaque nouveau client est désormais déployable en un à deux "
  "jours, contre plusieurs semaines au départ."),
 (23, "Conclusion & remerciements", "0:45", "FR",
  ...
  "poursuis en école d’ingénieur en alternance, en restant chez Axians. Je vous remercie de votre attention, "
  "et je suis à votre disposition pour vos questions."),
]

# --- 1) Notes du présentateur ---
prs = Presentation(PPTX)
slides = list(prs.slides)
assert len(slides) == len(SCRIPT), (len(slides), len(SCRIPT))
for sl, item in zip(slides, SCRIPT):
    sl.notes_slide.notes_text_frame.text = "[%s · %s] %s" % (item[2], item[3], item[4])
prs.save(PPTX)
...
NOTE=ParagraphStyle("NOTE",parent=TXT,textColor=AMBER,fontName="Helvetica-Oblique",fontSize=9)
...
def hf(c,d):
    ...
    c.drawRightString(W-MX,H-0.78*cm,"~22 min · dont 5 min EN · Ilyesse KEBAILI")
    c.setFillColor(GREY); c.setFont("Helvetica",7.5)
    c.drawString(MX,0.7*cm,"Minutage indicatif. Les diapos courtes (justifications) peuvent être accélérées si le temps presse.")
...
E.append(Spacer(1,4))
E.append(Paragraph("Ne lisez pas mot à mot : mémorisez les idées, gardez ce texte comme filet de sécurité. La transition "
    "vers l’anglais (diapo 4) et le retour au français (diapo 6) sont indiqués dans le texte. Les passages "
    "<i>en ambre</i> sont des notes personnelles — à NE PAS projeter ni dire au jury.",
    ParagraphStyle("i",parent=TXT,textColor=GREY)))
E.append(Spacer(1,6))

for (n,title,dur,lang,text) in SCRIPT:
    badge_col = GREEN if lang=="EN" else ACCENT
    meta = Table([[Paragraph("Diapo %d"%n,MET), Paragraph(dur,MET), Paragraph(lang,MET)]],
                 colWidths=[2.0*cm,2.0*cm,1.6*cm])
    ...
    head = Table([[meta, Paragraph(title,TIT)]],colWidths=[6.1*cm,FW-6.1*cm])
    ...
    for para in text.split("\n\n"):
        block.append(Paragraph(para.replace("\n"," "), NOTE if para.strip().startswith("[NOTE") else TXT))
